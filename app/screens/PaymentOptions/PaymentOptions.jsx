  ...
  return (
    <View className="flex-1 px-4 py-2 justify-between">
      <View className="flex-row flex-wrap gap-3 ml-3">
        {itemList.map((item, index) => (
          <Image
            key={index}
            source={item.image}
            className="w-[22%] h-[80px] rounded-xl mb-2 rounded-lg border border-gray-200"
          />
        ))}
      </View>
      ...
      <TouchableOpacity
        ...
        className="border border-green-600 rounded-lg py-4 mt-3 mb-8 flex-row justify-center items-center space-x-2"
      >
